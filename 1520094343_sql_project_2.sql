...
SELECT firstname, surname, joindate AS date_signup
FROM Members
...
the member name. */
SELECT DISTINCT(CONCAT(Members.firstname, " ", Members.surname)) AS Member_ID, Facilities.name
FROM Facilities
...
SELECT (CONCAT(Members.firstname, " ", Members.surname)) AS Member_ID, Facilities.name,
...
SELECT sub.Member_ID, sub.name AS facility, sub.total_cost
FROM 
	(SELECT (CONCAT(Members.firstname, " ", Members.surname)) AS Member_ID, Facilities.name,
	CASE WHEN Bookings.memid = 0 THEN Bookings.slots * Facilities.guestcost 
            ELSE Bookings.slots * Facilities.membercost END AS total_cost
